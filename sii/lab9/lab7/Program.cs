...
namespace lab7
{
    internal class Program
    {
        ...
        static List<Regex> types = [
            new(@"(\s*(?<nnot>не)?\s*квариумны(е|х|м))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*террариумны(е|х|м))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*птиц(ы|ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*млекопитающи(е|х|м))", RegexOptions.IgnoreCase)
        ];
        static List<Regex> species = [
            new(@"(\s*(?<nnot>не)?\s*(Рыб(ы|ам)?))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Рак(и|ов|ам))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Паук(и|ов|ам))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Зме(и|й|ям))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Ящер(ы|ов|иц|ам|ицам))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Попуга(и|ев|ям))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Кош(ки|ек|кам))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Собак(и|ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Кролик(и|ов|ам))", RegexOptions.IgnoreCase)
        ];
        static List<Regex> breeds = [
            new(@"(\s*(?<nnot>не)?\s*(Рыба(м)?-клоун(ам)?))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Моллинезия(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Гуппи)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Рыба(м)?-попуга(й|ям))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Креветка(м)? Блю Дрим)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Рак(ам)? Ябби)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Американски(й|м) Домашни(й|м) Паук(ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Черн(ая|ым) Вдова(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Полоз(ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Питон(ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Удав(ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Василиск(ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Плащеносн(ая|ым) ящерица(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Хамелеон(ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Игуана(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Волнисты(й|м) попуга(й|ям))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Неразлучник(и|ам))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Американск(ая|им) Короткошёрстн(ая|ым) кошка(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Бирманск(ая|им) кошка(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Британск(ая|им) короткошёрстн(ая|им) кошка(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Персидск(ая|им) кошка(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Сиамск(ая|им) кошка(м)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Джек-рассел-терьер(ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Мопс(ам)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Корги)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Алаба(й|ям))", RegexOptions.IgnoreCase),
        ...
        static List<Regex> dataNamesRegs = [
            new(@"(\s*(?<nnot>не)?\s*(Рыб(а|у)-клоун(а)?))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Моллинези(я|ю)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Гуппи)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Рыб(а|у)-попуга(й|я))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Креветк(а|у) Блю Дрим)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Рак(а)? Ябби)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Американск(ий|ого) Домашн(ий|его) паук(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Черн(ая|ую) вдов(а|у))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Полоз(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Питон(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Удав(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Василиск(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Плащеносн(ая|ую) ящериц(а|у))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Хамелеон(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Игуан(а|у)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Волнист(ый|ого) попуга(й|я))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Неразлучник(а)?1)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Неразлучник(а)?2)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Американск(ая|ую) Короткошёрстн(ая|ую) кошк(а|у))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Бирманск(ая|ую) кошк(а|у))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Британск(ая|ую) короткошёрстн(ая|ую) кошк(а|у))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Персидск(ая|ую) кошк(а|у))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Сиамск(ая|ую) кошк(а|у))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Джек-рассел-терьер(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Мопс(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Корги)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Алаба(й|я))", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Цветн(ой|ого) кролик(а)?)", RegexOptions.IgnoreCase),
            new(@"(\s*(?<nnot>не)?\s*Шиншилл(а|у))", RegexOptions.IgnoreCase)];
        ...
        static void Main(string[] args)
        {
            ...
            Regex classResetProp = new(@"сбрось фильтр на (?<prop>[\w\s]*)", RegexOptions.IgnoreCase);
            Regex classBack = new(@"(вернись на шаг назад)|(отмени последнее действие)", RegexOptions.IgnoreCase);
            Regex reset = new(@"Забыли|Сбрось результаты|Начни сначала|Начнем сначала|Начнем заново|Заново|(Сброс(ь)?)", RegexOptions.IgnoreCase);
            InitData();
            СходствоЖивотных RecommendSystem = new(tree);
            var AnimalsList = RecommendSystem.animals;
            List<Pet> curAnimalList = new(AnimalsList);
            List<List<Pet>> history = [];
            var inputString = "";
            do
            {
                history.Add(curAnimalList);
input:
                inputString = Console.ReadLine() ?? "";
                if (string.IsNullOrEmpty(inputString))
                    break;

                else if (classBack.IsMatch(inputString))
                {
                    try
                    {
                        curAnimalList = history[^2];
                    }
                    catch
                    {
                        Console.WriteLine("нет истории");
                        continue;                        
                    }
                    Console.WriteLine("текущий список животных");
                    foreach (var pet in curAnimalList)
                    {
                        Console.WriteLine($"{pet.Name}");
                    }
                }
                else if (classResetProp.IsMatch(inputString))
                {
                    var prop = classResetProp.Match(inputString).Groups.GetValueOrDefault("prop")!.Value.Trim().ToLower();
                    switch (prop)
                    {
                        case "условия содержания":
                            Filtration.Predicates.RemoveAll(p => p.Key == 1);
                            break;
                        case "цвет":
                            Filtration.Predicates.RemoveAll(p => p.Key == 2);
                            break;
                        case "тип":
                            Filtration.Predicates.RemoveAll(p => p.Key == 3);
                            break;
                        case "вид":
                            Filtration.Predicates.RemoveAll(p => p.Key == 4);
                            break;
                        case "породы":
                            Filtration.Predicates.RemoveAll(p => p.Key == 5);
                            break;
                        case "размер":
                            Filtration.Predicates.RemoveAll(p => p.Key == 6);
                            break;
                        case "возраст":
                            Filtration.Predicates.RemoveAll(p => p.Key == 7);
                            break;
                        case "продолжительность жизни":
                            Filtration.Predicates.RemoveAll(p => p.Key == 8);
                            break;
                        case "активность":
                            Filtration.Predicates.RemoveAll(p => p.Key == 9);
                            break;
                        case "вакцинацию":
                            Filtration.Predicates.RemoveAll(p => p.Key == 10);
                            break;
                    }
                    curAnimalList = AnimalsList.Where((pet) =>
                    {
                        bool res = true;
                        foreach (var filter in Filtration.Predicates)
                        {
                            res = filter.Value(pet, res);
                        }
                        return res;
                    }).ToList();
                    foreach (var pet in curAnimalList)
                ...
                else if (reset.IsMatch(inputString))
                {
                    curAnimalList = new(AnimalsList);
                ...
                else if (class2Extr.IsMatch(inputString))
                {
                    ...
                    curAnimalList = curAnimalList.Where((pet) =>
                    {
                        bool res = true;
                        foreach (var filter in Filtration.Predicates)
                        {
                    ...
                    var ordered = curAnimalList.OrderByDescending(pet => pet.Size);
                    if (cond.Contains("больш") || cond.Contains("больше всех"))
                        ordered = curAnimalList.OrderByDescending(pet => pet.Size);
                    if (cond.Contains("маленьк") || cond.Contains("меньше всех"))
                        ordered = curAnimalList.OrderBy(pet => pet.Size);
                    if (cond.Contains("стар") || cond.Contains("старше всех"))
                        ordered = curAnimalList.OrderByDescending(pet => pet.Age);
                    if (cond.Contains("молод") || cond.Contains("младше всех"))
                        ordered = curAnimalList.OrderBy(pet => pet.Age);
                    if (cond.Contains("живет дольше всех"))
                        ordered = curAnimalList.OrderByDescending(pet => pet.AvgAge);
                    if (cond.Contains("живет меньше всех"))
                        ordered = curAnimalList.OrderBy(pet => pet.AvgAge);
                ...
                else if (class23.IsMatch(inputString))
                {
                    ...
                    curAnimalList.Find(pet => likePetR.IsMatch(pet.Name))!.Like = notCond ? "-" : "+";

                    RecommendSystem = new(curAnimalList);
                    curAnimalList = RecommendSystem.Recommend();
                ...
                else if (class3better.IsMatch(inputString))
                {
                    ...
                    RecommendSystem = new(curAnimalList);
                    curAnimalList = RecommendSystem.Recommend();
                    var pet1I = curAnimalList.FindIndex(pet => pet1R.IsMatch(pet.Name));
                    var pet2I = curAnimalList.FindIndex(pet => pet2R.IsMatch(pet.Name));
                    if (pet1I < 0 && pet2I < 0)
                ...
                else if (class3similar.IsMatch(inputString))
                {
                    ...
                    do
                    {
                        ...
                        curAnimalList.Find(pet => petReg.IsMatch(pet.Name))!.Like = notCond ? "-" : "+";
                    }
                ...
                else if (class3LikeHard.IsMatch(inputString))
                {
                    ...
                    do
                    {
                        ...
                        curAnimalList.Find(pet => petReg.IsMatch(pet.Name))!.Like = "-";
                    }
                    while (!string.IsNullOrEmpty(head));
                    do
                    {
        ...
        static bool belongsChain = false;
        static void TypeFilter(Filter filtration, string op, Group? not, Group? filter)
        {
            if (!belongsChain && !belongsRegex.IsMatch(filter!.Value))
            {
                belongsChain = false;
                return;
            }
