    ...
    internal class Model
    {
        ...
        public double F0(double t)
        {
            //if (t < 100)
                //return 100;
            //else return 0;
            double f0 = Fmax / tmax * t * Math.Exp(-(t / tmax - 1));
            return f0;
        }
        ...
        {
            int i;
            for (i = 0; i < xs.Length && xval < xs[i]; i++);
            if (i >= xs.Length - 1) 
                i = xs.Length - 2;
            double yval = ys[i] + (ys[i + 1] - ys[i]) / (xs[i + 1] - xs[i]) * (xval - xs[i]);
            return yval;
        }
        ...
        public double[] RunMethod(double[] a, double[] b, double[] c, double[] d, double m0, double k0, double mn, double kn, double p0, double pn)
        {
            ...
            y[y.Length - 1] = T0; //(pn - mn * eta[eta.Length - 1]) / (kn + mn * xi[xi.Length - 1]);
