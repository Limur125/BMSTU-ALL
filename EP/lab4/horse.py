...
FACTORS_NUMBER = FACTORS_NUMBER_D4
FREE_AMOUNT = 1
...
# 70
COEFS_AMOUNT = FREE_AMOUNT + ONE_AMOUNT + TWO_AMOUNT + THREE_AMOUNT + FOUR_AMOUNT + SQUARE_AMOUNT
SMALL_COEFS_AMOUNT = FREE_AMOUNT + ONE_AMOUNT + TWO_AMOUNT + SQUARE_AMOUNT

n_SIZE_PFE = M_SIZE_D4
N_SIZE_OCKP = M_SIZE_D4 + 2 * FACTORS_NUMBER_D4 + 1
...
class Horse:
    ...
    def create_OCKP_plan_matrix(self):  # min_maxes: [[[min1, max1], [min2, max2], ...]
        ...
        xs_column_names = ['x0'] + \
                          ...
                          [''] * TWO_AMOUNT + \
                          [f'((x{factor_index})^2 - a)' for factor_index in range(1, FACTORS_NUMBER + 1)]
