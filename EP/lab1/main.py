...
class mywindow(QMainWindow):
    ...
    def graph_button_clicked(self):
        ...
        ax[0][0].plot(np.linspace(1, 4, len(t_mean_array)), np.multiply([1.5] * len(t_mean_array), t_mean_array))
        ax[0][0].set_title('Зависимость среднего время ожидания в очереди от интенсивности генератора\n'
                  f'({how_to_check}={end_param})')
        ax[0][0].grid()
        ax[0][0].set_ylabel('Сред1нее время ожидания в очереди (модельное время)')
        ax[0][0].set_xlabel('Интенсивность генератора')
        ax[0][1].plot(np.linspace(3, 5, len(t_mean_array) - 2), np.divide([2] *  (len(t_mean_array) - 2), t_mean_array[2:]))
        ax[0][1].set_title('Зависимость среднего время ожидания в очереди от интенсивности ОА\n'
                  f'({how_to_check}={end_param})')
        ax[0][1].grid()
        ax[0][1].set_ylabel('Среднее время ожидания в очереди (модельное время)')
        ax[0][1].set_xlabel('Интенсивность ОА')
        ax[1][0].plot(p_theor_array, t_mean_array)
        ax[1][0].set_title('Зависимость среднего время ожидания в очереди от загрузки системы\n'
                  f'({how_to_check}={end_param})')
        ax[1][0].grid()
        ax[1][0].set_ylabel('Среднее время ожидания в очереди (модельное время)')
        ax[1][0].set_xlabel('Загрузка системы')
        plt.show()
