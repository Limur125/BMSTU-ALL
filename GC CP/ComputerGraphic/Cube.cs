...
namespace ComputerGraphic
{
    ...
    internal class Cube : SceneObject
    {
        ...
        public Cube(Vector3[] v, int[][] poly, Color color, string name) : base(name)
        {
            vertices = v;
            polygons = poly;
            this.color = color;
        ...
        private Vector3 TriangleIntersection(Ray ray, Vector3[] tri)
        {
            ...
            if (g < 0)
                return new(float.PositiveInfinity);
            float alp = 1 - beta - g;
            return alp < 0 ? (new(float.PositiveInfinity)) : q;
        }
        public override Trace Intersection(Ray ray, LightSource l)
        {
            if (!shell.Intersection(ray))
                return new(new(float.PositiveInfinity), Color.Gray);
            ...
            foreach (var ipoly in polygons)
            {
                ...
                {
                    min = tmp;
                    cl = Intense(l.intens * Vector3.Dot(Vector3.Normalize(Vector3.Cross(poly[1] - poly[0], poly[2] - poly[0])), Vector3.Normalize(l.Position - min)));
                }
            }
            return float.IsFinite(min.X) ? (new(min, cl)) : (new(min, Color.Gray));
        }
        ...
        {
            Vector3 center = shell.Position;
            for (int i = 0; i < vertices.Length; i++)
            {
                double x, y, z;
                Vector3 vc = vertices[i] - center;
                y = (vc.Y * Math.Cos(ax)) - (vc.Z * Math.Sin(ax));
                z = (vc.Y * Math.Sin(ax)) + (vc.Z * Math.Cos(ax));
                vc.Y = (float)y;
                vc.Z = (float)z;
                x = (vc.X * Math.Cos(az)) - (vc.Y * Math.Sin(az));
                y = (vc.X * Math.Sin(az)) + (vc.Y * Math.Cos(az));
                vc.X = (float)x;
                vc.Y = (float)y;
                z = (vc.Z * Math.Cos(ay)) - (vc.X * Math.Sin(ay));
                x = (vc.Z * Math.Sin(ay)) + (vc.X * Math.Cos(ay));
                vc.Z = (float)z;
                vc.X = (float)x;
                vertices[i] = vc + center;
            }
        ...
        {
            Ray r = new(Vector3.Normalize(l - t.Point), t.Point);
