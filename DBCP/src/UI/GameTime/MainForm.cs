...
namespace GameTime
{
    public partial class MainForm : Form
    {
        ...
        {
            logger.LogTrace("{} Logout", DateTime.Now);
            LoginMessage.Text = "";
            EnterGuestMode();
            userLogin = "";
        }
        ...
        {
            logger.LogTrace("{} Authorization", DateTime.Now);
            LoginMessage.Text = "";
            ...
            if (login.Length < 3)
            {
                ...
                LoginMessage.Text = "Логин должен содержать не менее 3 символов";
                return;
            }
            if (password.Length < 3)
            {
                LoginMessage.ForeColor = Color.Red;
                LoginMessage.Text = "Пароль должен содержать не менее 3 символов";
                return;
            }
            ...
            catch (Exception ex)
            {
                ...
                LoginMessage.ForeColor = Color.Red;
                LoginMessage.Text = "Такого пользователя не существует";
                return;
            }
            EnterUserMode();
            ...
            userLogin = login;
            logger.LogTrace("{} Authorization complete", DateTime.Now);
        }

        private void RegisterButton_Click(object sender, EventArgs e)
        {
            logger.LogTrace("{} Registration", DateTime.Now);
            LoginMessage.Text = "";
            string login = LoginTextBox.Text;
            string password = PasswordTextBox.Text;
            if (login.Length < 3)
            ...
            {
                LoginMessage.ForeColor = Color.Red;
                LoginMessage.Text = "Такой пользователь уже существует";
            ...
            LoginMessage.ForeColor = Color.Black;
            userLogin = login;
            logger.LogTrace("{} Registration complete", DateTime.Now);
        }
        void EnterAdminMode()
        {
            logger.LogTrace("{} Admin mode", DateTime.Now);
            mode = UserMode.ADMIN;
        ...
        {
            logger.LogTrace("{} Delete game \"{}\"", DateTime.Now, currentGame.Game.Title);
            try
            {
            ...
            {
                logger.LogError("{} Delete game Failed\n{}\n{}", DateTime.Now, ex.Message, ex.StackTrace);
            }
        ...
        public void SaveTimeRecord(int type, string hoursStr, string minutesStr)
        {
            ...
            {
                logger.LogError("{} SaveTimeRecord convert type failed\n{}\n{}", DateTime.Now, e.Message, e.StackTrace);
                MessageBox.Show("Некорректно введен тип временной отметки");
                return;
            }
            int hours, minutes;
            try
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Некорректно введены минуты или часы");
                return;
            }
        ...
        private void AllReviewsButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                reviews = reviewService.GetReviews(currentGame.Game.Id);
            } catch { }
        ...
        private void AllTimeRecords_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                records = timeRecordService.GetTimeRecords(currentGame.Game.Id);
            } catch { }
