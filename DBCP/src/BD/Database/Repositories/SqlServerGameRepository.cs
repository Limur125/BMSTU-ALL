...
namespace SqlServerDatabase
{
    public class SqlServerGameRepository : SqlServerRepository, IGameRepository
    {
        ...
        XmlDocument GetXml(string genre)
        {
            string[] genreSplit = genre.Split(' ');
            XmlDocument xml = new XmlDocument();
            XmlElement root = xml.CreateElement("Genre");
            xml.AppendChild(root);
            foreach (var platf in genreSplit)
            {
                xml["Genre"].AppendChild(xml.CreateElement("genre")).InnerText = platf;
            }
            return xml;
        }
        ...
        public Game GetGame(string title)
        {
            ...
            if (gameTable.Where(g => g.Title == title).Count() == 0)
                throw new GameNotFoundException("Game not found");
            string genreS = game.Platform;
            try
            {
                var xdoc = XDocument.Parse(game.Platform);
                var items = from xe in xdoc.Root.Elements("genre")
                            select xe.Value;
                genreS = string.Join(' ', items);
            }
            catch { }
            
            return new Game(game.Id, game.Title, game.ReleaseDate, game.Developer, game.Publisher, genreS);
        }
    ...
    public class InsertGameProcedure : DataContext
    {
        ...
        public int InsertGamesXML(
            ...
            [Parameter(Name = "genre", DbType = "nvarchar(200)")] string platform
            )
        {
